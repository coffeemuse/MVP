...
DO WHILE LENGTH(ARGUMENTS) > 0
  ...
  IF (P = "-D" | P = '--DEBUG') THEN DO
    CALL SETG('DEBUG_ON',1)
    CALL DEBUG "DEBUGGING ENABLED"
  END
  IF (P = "-FORCE" | P = '--FORCE') THEN DO
    CALL SETG('FORCE',1)
    CALL DEBUG "FORCE ENABLED"
  END
END
...
INSTALL:
  ...
  IF ALREADY_INSTALLED THEN DO
    IF GETG('FORCE') = 1 THEN DO
      SAY PACKAGE "IS ALREADY INSTALLED, REINSTALLING (-FORCE)"
    END
    ELSE DO
      SAY PACKAGE "IS ALREADY INSTALLED"
      SAY "USE: RX MVP INSTALL" PACKAGE "-FORCE TO REINSTALL"
      RETURN
    END
  END
  ...
  DO WHILE REVDEP \= ''
    ...
    /* -FORCE ONLY REINSTALLS THE REQUESTED PACKAGE, NOT ITS DEPS */
    IF CHECK_INSTALLED(IDEP) THEN DO
      IF GETG('FORCE') = 0 THEN ITERATE
      IF IDEP \= PACKAGE THEN ITERATE
    END
    TOTAL = TOTAL + 1
    DEPSTEM.TOTAL = IDEP
...
MARK_INSTALLED:
  ...
  /* ON A -FORCE REINSTALL THE PACKAGE MAY ALREADY BE IN THE DB,
     DON'T ADD A DUPLICATE ENTRY */
  DO MI=1 TO MVPDB.0
    IF WORD(MVPDB.MI,1) = M_INSTALLED & WORD(MVPDB.MI,2) = M_VERSION THEN DO
      CALL DEBUG "MARK_INSTALLED:" M_INSTALLED M_VERSION "ALREADY IN DB"
      RETURN
    END
  END
...
/* RETURNS NOTHING */
  ...
  DO J=1 TO WORKJCL.0
    ...
    /* REMEMBER IF THE JOB CARD ALREADY SETS A TIME LIMIT */
    IF POS('TIME=',TRANSLATE(WORKJCL.J)) > 0 THEN HASTIME = 1
    ...
    /* TIME=1440 = NO CPU LIMIT: INSTALL JOBS (DISK FORMATS, BIG
       RECEIVES) CAN LEGITIMATELY RUN LONGER THAN THE CLASS DEFAULT.
       SKIPPED IF THE PACKAGE JOB CARD ALREADY SETS ITS OWN TIME=. */
    IF HASTIME THEN
      QUEUE "//    USER=MVP,PASSWORD="||GET_PW()
    ELSE
      QUEUE "//    USER=MVP,PASSWORD="||GET_PW()||",TIME=1440"
  ...
  DO X=1 TO QUEUED()
    /* PARSE PULL, NOT PULL: PULL = PARSE UPPER PULL AND WOULD
       UPPERCASE INLINE DD DATA (PARMLIB CONFIGS) IN THE JOB */
    PARSE PULL ELEMENT
    JCLSTEM.X = ELEMENT
  END
  JCLSTEM.0 = TOTAL                                     
...
USAGE:
  ...
  SAY "  RX MVP INSTALL PACKAGE -FORCE"
  SAY ""
  ...
  SAY "  -FORCE     - WITH INSTALL, REINSTALL EVEN IF ALREADY INSTALLED"
  RETURN
